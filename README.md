...
市面上BTC,ETC,LTC(DOGE),ZEC,DASH损耗最小的运维软件(中转,矿机管理,加密,抽水)
...
# 更新日志
    6.0.0 2026年9月1日
   【重磅】新增无损抽水模式，部分矿池可 0 损耗抽水，再也不会因为任务切换、难度变化导致客户算力发生损耗（端口配置-抽水配置-无损模式处开启）

   【重磅】新增 RMS3(NB)协议，仅支持 BTC、LTC，公网流量体积最大可节约 95%+，大幅改善恶劣网络环境下表现，完全在网络上隐身您的矿场。（此协议需更新 RMS 本地客户端至3.2.0版本）

   移除了 RUSTMINERSYSTEM 以及 RMS 的超级压缩模式，且修复了 RMS3 带来的拒绝率提升问题，以及部分机型无法接入问题。

   增加矿机掉线清理时间设置

   重做了替换钱包、矿工以及重定向矿池的功能，现在可灵活调配，且满足所有组合

   修复了 RUST、RMS 流量统计单位不正常的问题

   5.0.3 2026年7月
   强制下发成功份额配置处增加延迟下发间隔，在程序与矿机处于同一局域网等近距离部署时，双方延迟可能接近 0ms，部分蚂蚁矿机固件可能将过快的成功响应误判为 stale 份额，可酌情增加少量延迟规避。远程部署一般无需设置。
   页面细节优化
   5.0.0 2026年6月
   大幅更新了UI，增加APP远程查看功能
```4.6.2 2026年1月13
   【重要更新】修复近几个版本矿机以及端口上图表的份额会一直累加的bug
    增加透明代理协议
    修复rms推送地址拉不到zstd算法端口的bug
